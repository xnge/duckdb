...
U_CFUNC int32_t locale_getKeywords(const char *localeID, char prev, char *keywords, int32_t keywordCapacity,
                                   char *values, int32_t valuesCapacity, int32_t *valLen, UBool valuesToo,
                                   UErrorCode *status);
...
Locale::Locale() : UObject(), fullName(fullNameBuffer), baseName(NULL) {
	init(NULL, FALSE);
}
 ...
 *   the default locale.)
 */
Locale::Locale(Locale::ELocaleType) : UObject(), fullName(fullNameBuffer), baseName(NULL) {
	setToBogus();
}
...
>>>>>>> 62ec77a03 (CheckKeyValidity -> AreKeysNull and reworked the structure of the MapFunction a bit)
...
Locale &Locale::init(const char *localeID, UBool canonicalize) {
	...
	do {
		...
		if (err == U_BUFFER_OVERFLOW_ERROR || length >= (int32_t)sizeof(fullNameBuffer)) {
			...
			err = U_ZERO_ERROR;
			length = canonicalize ? uloc_canonicalize(localeID, fullName, length + 1, &err)
		...
		if (fieldLen[1] == 4 && ISASCIIALPHA(field[1][0]) && ISASCIIALPHA(field[1][1]) && ISASCIIALPHA(field[1][2]) &&
...
StringEnumeration *Locale::createKeywords(UErrorCode &status) const {
	...
	if (variantStart) {
		if (assignment > variantStart) {
			int32_t keyLen =
			    locale_getKeywords(variantStart + 1, '@', keywords, keywordCapacity, NULL, 0, NULL, FALSE, &status);
...
void Locale::getKeywordValue(StringPiece keywordName, ByteSink &sink, UErrorCode &status) const {
	...
	for (;;) {
		...
		buffer = sink.GetAppendBuffer(
		    ...
		    /*desired_capacity_hint=*/scratch_capacity, scratch.getAlias(), scratch_capacity, &result_capacity);

		reslen = uloc_getKeywordValue(fullName, keywordName_nul.data(), buffer, result_capacity, &status);
